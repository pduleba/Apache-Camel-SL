package com.pluralsight.processor.webmvc;
...
public class OrderController {
	...
	public String resetOrders(Model model) {
		List<OrderDataDTO> orders = orderService.getOrderDetails(
				OrderStatusDTO.PROCESSING, 100);
		try {
			orderService.processOrderStatusUpdate(orders, OrderStatusDTO.NEW);
		} catch (Exception e) {
			log.error(
					"An error occurred while resetting orders: "
							+ e.getMessage(), e);
		}
		model.addAttribute("orders", orderService.getOrderDetails());
		return "viewOrders";
